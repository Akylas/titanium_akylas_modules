...
{
    [wetPaintView setMultipleTouchEnabled:[TiUtils boolValue:args]];
}
...
{
    if ([[self viewProxy] _hasListeners:@"onpaint" checkParent:NO])
    {
        NSMutableArray* result =[NSMutableArray array];
        [[wetPaintView path] enumerateObjectsUsingBlock:^(NSValue* obj, NSUInteger idx, BOOL *stop) {
            CGPoint point = [obj CGPointValue];
            [result addObject:@[@(point.x), @(point.y)]];
        }];
        [self.proxy fireEvent:@"onpaint" withObject:@{
                                                        @"path":result

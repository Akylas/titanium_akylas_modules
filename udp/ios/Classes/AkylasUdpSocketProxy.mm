...
@interface AkylasUdpSocketProxy (Private)
...
-(void)start:(id)args
{
    ...
    listening = [TiUtils boolValue:[args objectForKey:@"listening"] def:true];
...
-(void)_send:(NSData*)data withDict:(NSDictionary*)args
{
    ...
    if (err == 0)
    {
        NSLog ( @"data sent2: %d", ([NSDate timeIntervalSinceReferenceDate] * 1000) );
    }
    else
    {
...
{
    AkylasUdpSocketProxy* obj = (AkylasUdpSocketProxy*)info;
    [obj _readData];
}

// This C routine is called by CFSocket when there's data waiting on our 
// UDP socket.  It just redirects the call to Objective-C code.
static void SocketWriteCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void* data, void* info)
{
//    AkylasUdpSocketProxy* obj = (AkylasUdpSocketProxy*)info;
//    [obj _readData];
}
...
-(BOOL)_setupSocketConnectedToAddress:(NSData*)address port:(NSUInteger)port error:(NSError**)errorPtr
{
    ...
    if (err == 0)
    {
        if (listening)
        {
            NSLog(@"creating socket for listening");
        ...
        {
            NSLog(@"creating socket for sending");
            self->_cfSocket = CFSocketCreateWithNative(NULL, sock, kCFSocketWriteCallBack, SocketWriteCallback, &context);

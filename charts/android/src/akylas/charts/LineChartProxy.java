...
import akylas.charts.XYSerieProxy.AkXYSeries;
...
public class LineChartProxy extends ChartProxy {
    ...
    private class LineChartView extends ChartView implements OnTouchListener {
        ...
        public void updateGradient(final boolean force) {
            if (force || needsUpdateGradients == true) {
                ...
                Rect rect = new Rect();
                ((XYPlot) plotView).getGraphWidget()
                        .getWidgetDimensions().paddedRect.round(rect);
        ...
        public void initBarRenderer() {
            ...
            if (d.containsKey("barPlot")) {
                ...
                if (barPlotOptions.containsKey("barStyle")) {
                    renderer.setBarRenderStyle(BarRenderStyle
                            .values()[barPlotOptions.getInt("barStyle")]);
                }
                if (barPlotOptions.containsKey("barWidthStyle")) {
                    renderer.setBarWidthStyle(BarWidthStyle
                            .values()[barPlotOptions.getInt("barWidthStyle")]);
                }
            ...
            for (XYSeries series : xyPlotView.getSeriesSet()) {
                ...
                synchronized (series) {
                    for (int i = 0; i < series.size(); i++) {
                        ...
                        if (thisX != null && (minXSeriesValue == null
                                || thisX.doubleValue() < minXSeriesValue
                                        .doubleValue())) {
                            minXSeriesValue = thisX;
                        }

                        // Number thisMaxX = series.getMaxX();
                        if (thisX != null && (maxXSeriesValue == null
                                || thisX.doubleValue() > maxXSeriesValue
                                        .doubleValue())) {
                            maxXSeriesValue = thisX;
                        }

                        // Number thisMinY = series.getMinY();
                        if (thisY != null && (minYSeriesValue == null
                                || thisY.doubleValue() < minYSeriesValue
                                        .doubleValue())) {
                            minYSeriesValue = thisY;
                        }

                        // Number thisMaxY = series.getMaxY();
                        if (thisY != null && (maxYSeriesValue == null
                                || thisY.doubleValue() > maxYSeriesValue
                                        .doubleValue())) {
        ...
        protected void updateGradients(Context context, Rect rect) {
            ...
            if (d.containsKey(AkylasChartsModule.PROPERTY_PLOT_AREA)) {
                KrollDict gridOptions = d
                        .getKrollDict(AkylasChartsModule.PROPERTY_PLOT_AREA);
                if (gridOptions.containsKey(TiC.PROPERTY_BACKGROUND_GRADIENT)) {
                    ...
                    xyPlotView.getGraphWidget().getOrCreateGridBackgroundPaint()
                            .setShader(Utils.styleGradient(bgOptions, context,
                                    rect));
                }
            }
        ...
        public void propertySet(String key, Object newValue, Object oldValue,
            ...
            case AkylasChartsModule.PROPERTY_PLOT_SPACE:
                ...
                boolean scaleToFit = plotOptions.optBoolean(
                        AkylasChartsModule.PROPERTY_SCALE_TO_FIT, false);
                if (scaleToFit) {
                    xyPlotView.setRangeBoundaries(0, 0, BoundaryMode.AUTO);
                    xyPlotView.setDomainBoundaries(0, 0, BoundaryMode.AUTO);
                } else {
                    ...
                    if (plotOptions
                            .containsKey(AkylasChartsModule.PROPERTY_RANGE_X)) {
                        KrollDict xOptions = plotOptions.getKrollDict(
                                AkylasChartsModule.PROPERTY_RANGE_X);
                    ...
                    if (plotOptions
                            .containsKey(AkylasChartsModule.PROPERTY_RANGE_Y)) {
                        KrollDict yOptions = plotOptions.getKrollDict(
                                AkylasChartsModule.PROPERTY_RANGE_Y);
                        if (yOptions.containsKey(TiC.PROPERTY_MIN)
                                && yOptions.containsKey(TiC.PROPERTY_MAX)) {
                            double min = yOptions.getDouble(TiC.PROPERTY_MIN);
                            double max = yOptions.getDouble(TiC.PROPERTY_MAX);
                ...
                break;
            case AkylasChartsModule.PROPERTY_PLOT_AREA: {
                ...
                Utils.styleOpacity(gridOptions, TiC.PROPERTY_BACKGROUND_OPACITY,
                        paint1);
            ...
            case AkylasChartsModule.PROPERTY_AXIS_X: {
                ...
                if (currentOptions != null) {
                    // axisOptions = KrollDict.merge(currentOptions,
                    // axisOptions);
                ...
                if (axisOptions
                        .containsKey(AkylasChartsModule.PROPERTY_ORIGIN)) {
                    xyPlotView.setUserDomainOrigin((Number) axisOptions
                            .get(AkylasChartsModule.PROPERTY_ORIGIN));
                }

                if (axisOptions
                        .containsKey(AkylasChartsModule.PROPERTY_ALIGN)) {
                    xyPlotView.getGraphWidget().setRangeAxisAlignment(
                            Utils.gravityFromAlignment(axisOptions.getInt(
                                    AkylasChartsModule.PROPERTY_ALIGN)));
                }

                Paint paint = xyPlotView.getGraphWidget()
                        .getOrCreateRangeOriginLinePaint();
                Utils.styleColor(axisOptions,
                        AkylasChartsModule.PROPERTY_LINE_COLOR,
                        Color.TRANSPARENT, paint);
                Utils.styleStrokeWidth(axisOptions,
                        AkylasChartsModule.PROPERTY_LINE_WIDTH, paint, context);

                if (axisOptions.containsKey(TiC.PROPERTY_TITLE)) {
                    KrollDict titleOptions = axisOptions
                            .getKrollDict(TiC.PROPERTY_TITLE);
                    if (titleOptions.containsKey(TiC.PROPERTY_TEXT)) {
                        xyPlotView.setDomainLabel(
                                titleOptions.getString(TiC.PROPERTY_TEXT));
                    }

                    Utils.styleTextWidget(titleOptions,
                            xyPlotView.getGraphWidget()
                                    .getOrCreateDomainOriginLabelPaint(),
                            context);
                    if (titleOptions.containsKey(TiC.PROPERTY_OFFSET)) {
                        xyPlotView.getGraphWidget().setDomainOriginLabelOffset(
                                Utils.getRawSizeOrZero(titleOptions,
                                        TiC.PROPERTY_OFFSET, context));
                    }
                    if (titleOptions
                            .containsKey(AkylasChartsModule.PROPERTY_ANGLE)) {
                        xyPlotView.getGraphWidget().setDomainOriginLabelAngle(
                                Utils.getRawSizeOrZero(titleOptions,
                                        AkylasChartsModule.PROPERTY_ANGLE,
                                        context));
                    }
                }
                ...
                if (axisOptions
                        .containsKey(AkylasChartsModule.PROPERTY_MINOR_TICKS)) {
                    KrollDict minorOptions = axisOptions.getKrollDict(
                            AkylasChartsModule.PROPERTY_MINOR_TICKS);
                    if (minorOptions.containsKey(TiC.PROPERTY_COUNT)) {
                        ticksPerDomain = minorOptions
                                .getInt(TiC.PROPERTY_COUNT);
                    }
                    if (minorOptions.containsKey(
                            AkylasChartsModule.PROPERTY_GRID_LINES)) {
                        KrollDict gridOptions = minorOptions.getKrollDict(
                                AkylasChartsModule.PROPERTY_GRID_LINES);
                        Paint paint1 = xyPlotView.getGraphWidget()
                                .getOrCreateDomainSubGridLinePaint();

                        Utils.styleCap(gridOptions,
                                AkylasChartsModule.PROPERTY_CAP, paint1);
                        Utils.styleJoin(gridOptions,
                                AkylasChartsModule.PROPERTY_JOIN, paint1);
                        Utils.styleEmboss(gridOptions,
                                AkylasChartsModule.PROPERTY_EMBOSS, paint1);
                        Utils.styleDash(gridOptions,
                                AkylasChartsModule.PROPERTY_DASH, paint1);
                        Utils.styleShadow(gridOptions,
                                AkylasChartsModule.PROPERTY_SHADOW, paint1);
                        Utils.styleColor(gridOptions, TiC.PROPERTY_COLOR,
                                Color.TRANSPARENT, paint1);
                        Utils.styleStrokeWidth(gridOptions, paint1, context);
                        Utils.styleOpacity(gridOptions, paint1);
                    }
                }
                xyPlotView.setDomainStep(XYStepMode.AUTOMATIC, 0);
                xyPlotView.setRangeStep(XYStepMode.AUTOMATIC, 0);

                if (axisOptions
                        .containsKey(AkylasChartsModule.PROPERTY_INTERVAL_PX)) {
                    double interval = axisOptions
                            .getDouble(AkylasChartsModule.PROPERTY_INTERVAL_PX);
                ...
                if (axisOptions
                        .containsKey(AkylasChartsModule.PROPERTY_MAJOR_TICKS)) {
                    KrollDict majorOptions = axisOptions.getKrollDict(
                            AkylasChartsModule.PROPERTY_MAJOR_TICKS);

                    if (majorOptions.containsKey(
                            AkylasChartsModule.PROPERTY_INTERVAL)) {
                        double interval = majorOptions.getDouble(
                                AkylasChartsModule.PROPERTY_INTERVAL);
                        xyPlotView.setTicksPerDomainLabel((int) interval);
                    ...
                    if (majorOptions
                            .containsKey(AkylasChartsModule.PROPERTY_LABELS)) {
                        Paint labelPaint = xyPlotView.getGraphWidget()
                                .getOrCreateDomainLabelPaint();
                        KrollDict labelOptions = majorOptions.getKrollDict(
                                AkylasChartsModule.PROPERTY_LABELS);
                        Utils.styleTextWidget(labelOptions, labelPaint,
                                context);

                        if (labelOptions.containsKey(
                                AkylasChartsModule.PROPERTY_FORMAT_CALLBACK)) {
                        ...
                        if (formatDomainCallback != null) {
                            ...
                            if (labelOptions.containsKey(
                                    AkylasChartsModule.PROPERTY_LOCATIONS)) {
                                ...
                                if (locations != null) {
                                    for (int i = 0; i < locations.length; i++) {
                                        ...
                                        if (value != null && text != null) {
                                            xyPlotView.getGraphWidget()
                                                    .addDomainValueFormat(value,
                                                            text);
                                        }
                                    }
                                }
                            }
                            Utils.styleValueFormat(labelOptions,
                                    xyPlotView.getGraphWidget(),
                    ...
                    if (majorOptions.containsKey(
                            AkylasChartsModule.PROPERTY_GRID_LINES)) {
                        KrollDict gridOptions = majorOptions.getKrollDict(
                                AkylasChartsModule.PROPERTY_GRID_LINES);
                        Paint paint1 = xyPlotView.getGraphWidget()
                ...
                if (axisOptions.containsKey(
                        AkylasChartsModule.PROPERTY_VISIBLE_RANGE)) {
                    KrollDict visibleRangeOptions = axisOptions.getKrollDict(
                            AkylasChartsModule.PROPERTY_VISIBLE_RANGE);
                    if (visibleRangeOptions.containsKey(TiC.PROPERTY_MIN)) {
                        xyPlotView.setDomainLeftMin(
                                visibleRangeOptions.getFloat(TiC.PROPERTY_MIN));
                    }
                    if (visibleRangeOptions.containsKey(TiC.PROPERTY_MAX)) {
                        xyPlotView.setDomainRightMax(
                                visibleRangeOptions.getFloat(TiC.PROPERTY_MAX));
                    }
                }
            ...
            case AkylasChartsModule.PROPERTY_AXIS_Y: {
                ...
                if (axisOptions
                        .containsKey(AkylasChartsModule.PROPERTY_ALIGN)) {
                    xyPlotView.getGraphWidget().setDomainAxisAlignment(
                            Utils.gravityFromAlignment(axisOptions.getInt(
                                    AkylasChartsModule.PROPERTY_ALIGN)));
                }
                Paint paint = xyPlotView.getGraphWidget()
                ...
                if (axisOptions.containsKey(TiC.PROPERTY_TITLE)) {
                    KrollDict titleOptions = axisOptions
                            .getKrollDict(TiC.PROPERTY_TITLE);
                    if (titleOptions.containsKey(TiC.PROPERTY_TEXT)) {
                        xyPlotView.setRangeLabel(
                                titleOptions.getString(TiC.PROPERTY_TEXT));
                    }

                    Utils.styleTextWidget(titleOptions,
                            xyPlotView.getGraphWidget()
                                    .getOrCreateRangeOriginLabelPaint(),
                            context);
                    if (titleOptions.containsKey(TiC.PROPERTY_OFFSET)) {
                        xyPlotView.getGraphWidget().setRangeOriginLabelOffset(
                                Utils.getRawSizeOrZero(titleOptions,
                                        TiC.PROPERTY_OFFSET, context));
                    }
                    if (titleOptions
                            .containsKey(AkylasChartsModule.PROPERTY_ANGLE)) {
                        xyPlotView.getGraphWidget().setRangeOriginLabelAngle(
                                Utils.getRawSizeOrZero(titleOptions,
                                        AkylasChartsModule.PROPERTY_ANGLE,
                                        context));
                    }
                ...
                if (axisOptions
                    ...
                    xyPlotView.setRangeStep(XYStepMode.INCREMENT_BY_PIXELS,
                            axisOptions.getDouble(
                                    AkylasChartsModule.PROPERTY_INTERVAL_PX));
                }
                ...
                if (axisOptions.containsKey(
                    ...
                    KrollDict visibleRangeOptions = axisOptions.getKrollDict(
                            AkylasChartsModule.PROPERTY_VISIBLE_RANGE);
                    if (visibleRangeOptions.containsKey(TiC.PROPERTY_MIN)) {
                        xyPlotView.setRangeBottomMin(
                                visibleRangeOptions.getFloat(TiC.PROPERTY_MIN));
                    }
                    if (visibleRangeOptions.containsKey(TiC.PROPERTY_MAX)) {
                        xyPlotView.setRangeTopMax(
                                visibleRangeOptions.getFloat(TiC.PROPERTY_MAX));
                    }
                }
            ...
            case MotionEvent.ACTION_MOVE:
                if (mode == ONE_FINGER_DRAG && panEnabled) {
                    ...
                    if (oldDist > 0 && newDist < 0
                            || oldDist < 0 && newDist > 0) // sign change!
                                                           // Fingers have
                                                           // crossed
                                                           // ;-)
                        break;
        ...
        private boolean scroll(float pan) {
            ...
            if (clampInteraction) {
                if ((newMinX.floatValue() < minXSeriesValue.floatValue()
                        && newMaxX.floatValue() <= maxXSeriesValue
                                .floatValue())) {
        ...
        private boolean clamp(float testNewMnX, float testNewMaxX) {
            if (clampInteraction) {
                if ((testNewMnX < minXSeriesValue.floatValue()
                        && testNewMaxX <= maxXSeriesValue.floatValue())
                        || (testNewMnX >= minXSeriesValue.floatValue()
                                && testNewMaxX > maxXSeriesValue.floatValue()))
                    return true;
            }
            return false;
    ...
    public MarkerProxy addMarker(Object object) {
        ...
        if (object instanceof HashMap) {
            ...
            marker = (MarkerProxy) KrollProxy.createProxy(MarkerProxy.class,
                    null, new Object[] { object }, null);
    ...
    public final Rect getGraphRect() {
        Rect rect = new Rect();
        if (xyPlotView != null) {
            xyPlotView.getGraphWidget().getWidgetDimensions().paddedRect
                    .round(rect);
        }
        return rect;
    }

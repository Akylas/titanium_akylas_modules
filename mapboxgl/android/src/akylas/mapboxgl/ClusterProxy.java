...
@Kroll.proxy(creatableInModule = AkylasMapboxGLModule.class, propertyAccessors = {
...
//                public void execute() {
                    ((AkMapView) mapView).getClusterManager().clusterAlgo(_algorithm);
//                }
//            }, false);
        }
    }
    ...
    private Icon iconFromBitmap(Bitmap image) {
        return IconFactory.getInstance(TiApplication.getAppContext()).fromBitmap(image);
    }
    ...
    public void prepareClusterMarkerOptions(Cluster<AnnotationProxy> cluster,
        ...
        Icon icon = iconFromBitmap(generateBitmap(cluster));
    ...
    public void removeFromMap() {
        if (mapView != null) {
            ((AkMapView) mapView).getClusterManager().removeClusterAlgorithm(_algorithm);
        }
    }
}

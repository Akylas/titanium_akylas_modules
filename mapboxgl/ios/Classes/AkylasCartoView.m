...
@implementation CartoMapEventListener

-(void) onMapClicked:(NTMapClickInfo *)mapClickInfo
{
    [_mapView onMapClicked:mapClickInfo];
}
-(void)onMapMoved {
    [_mapView onMapMoved];
}
...
-(void)onMapStable {
    [_mapView onMapStable];
}
@end


@implementation AkylasNTMapView
{
}
-(void)dealloc
{
...
@property (nonatomic, readwrite, retain) NTVectorElement* overlay;
@end
...
@implementation AkylasCartoView
{
    AkylasNTMapView *map;
    BOOL _forwarding;
...
//    GClusterManager* _clusterManager;
    NTVectorElement* _selectedOverlay;
    ...
    GLKViewController* glController;
    CartoMapEventListener* _eventlistener;
    
    UIEdgeInsets _padding;
    
    NTVectorLayer* markersLayer;
}

-(void)releaseGLKController {
    if (glController) {
//        glController.searchResultsDataSource = nil;
//        glController.searchResultsDelegate = nil;
        glController.delegate = nil;
        RELEASE_TO_NIL(glController)
    }
}

-(GLKViewController*)glController {
    if (!glController) {
        glController = [[GLKViewController alloc] init];
        // GLKViewController-specific parameters for smoother animations
        [glController setResumeOnDidBecomeActive:NO];
        [glController setPreferredFramesPerSecond:60];
    }
}
...
//    RELEASE_TO_NIL(_clusterManager)
    RELEASE_TO_NIL(_eventlistener);
    RELEASE_TO_NIL(_selectedOverlay)
    [self releaseGLKController];
	[super dealloc];
}
...
-(AkylasNTMapView*)map
{
    ...
    {
//        @try {
            map = [[AkylasNTMapView alloc] initWithFrame:self.bounds];
...
//        map.settings.allowScrollGesturesDuringRotateOrZoom = YES;
        _eventlistener = [[CartoMapEventListener alloc] init];
        _eventlistener.mapView = self;
        [map setMapEventListener:_eventlistener];
        [[map getOptions] setDPI:[TiUtils dpi]];
        //Initialize loaded state to YES. This will automatically go to NO if the map needs to download new data
        loaded = YES;
        
        NTLocalVectorDataSource* source = [[NTLocalVectorDataSource alloc] initWithProjection:[[map getOptions] getBaseProjection]];
        markersLayer = [[NTVectorLayer alloc] initWithDataSource:[source autorelease]];
...
//    {
//        [self map];
//        _clusterManager = [[GClusterManager managerWithMapView:map renderer:[[[AkylasCartoClusterRenderer alloc] initWithMapView:map] autorelease]] retain];
...
-(CLLocationCoordinate2D)centerCoordinate {
    NTMapPos* pos = [[self map] getFocusPos];
    return CLLocationCoordinate2DMake([pos getY], [pos getX]);
}
...
-(id)metersPerPixel_
{
    ...
    TiThreadPerformBlockOnMainThread(^{
        NTProjection *projection = [[[self map] getOptions] getBaseProjection];
       px = [projection fromInternalScale:1];
    }, YES);
...
-(void)setRegion_:(id)value
{
    ...
    AkRegion region = [AkylasCartoModule regionFromObject:value];
...
//        }
//    } else {
        NTMapBounds* position = boundsFromRegion(region);
//        if (animate || !configurationSet) {
            [[self map] moveToFitBounds:position screenBounds:nil integerZoom:false durationSeconds:animate?1:0];
...
//-(id)userLocation_
//{
//    return [AkylasCartoModule dictFromLocation:[self map].myLocation];
//}
...
//        id obj = [[AkylasCartoModule MapType] objectForKey:value];
//        if (obj) {
...
//    if ([self shouldAnimate]) {
        [[self map] setZoom:newValue durationSeconds:[self shouldAnimate]?1:0];
...
-(void)setBearing_:(id)value
{
    ...
    [[self map] setRotation:newValue durationSeconds:[self shouldAnimate]?1:0];
...
-(void)setTilt_:(id)value
{
    CGFloat newValue = [TiUtils floatValue:value def:0];
    ENSURE_UI_THREAD_1_ARG(value)
    [[self map] setTilt:newValue durationSeconds:[self shouldAnimate]?1:0];
//    if ([self shouldAnimate]) {
...
//    [self setShouldFollowUserLocation:!configurationSet];
    CLLocationCoordinate2D coord = [AkylasCartoModule locationFromObject:value];
    [self setCenterCoordinate:coord animated:[self shouldAnimate]];
}

-(id)centerCoordinate_
{
    return [AkylasCartoModule dictFromLocation2D:[self centerCoordinate]];
}

-(void)setMinZoom_:(id)zoom
{
    NTOptions* options = [[self map] getOptions];
    CGFloat min = [TiUtils floatValue:zoom def:0];
    CGFloat max = [[options getZoomRange] getMax];
    CGFloat oldValue = [[options getZoomRange] getMin];
    
    [options setZoomRange:[[NTMapRange alloc] initWithMin:min max:max]];
...
-(void)setMaxZoom_:(id)zoom
{
    ...
    CGFloat min = [[options getZoomRange] getMin];
    CGFloat max = [TiUtils floatValue:zoom def:23];
    CGFloat oldValue = [[options getZoomRange] getMax];
...
//{
//    [[[self map] getOptions] setSeamlessPanning:[TiUtils boolValue:value]];
...
{
        [[[self map] getOptions] setRotatable:[TiUtils boolValue:value]];
}

//-(void)setTiltEnabled_:(id)value
//{
//    [[[self map] getOptions] setTilt:[TiUtils boolValue:value]];
//}
...
{
    [self glController].preferredFramesPerSecond = [TiUtils intValue:value];
}
...
- (void)zoomInAt:(CGPoint)pivot animated:(BOOL)animated
{
    ...
    [[self map] zoom:[[self map] getZoom] targetPos:<#(NTMapPos *)#> durationSeconds:<#(float)#>
...
-(void)selectAnnotation:(id)args
{
    ...
    if (IS_OF_CLASS(overlay, NTMarker)) {
        map.selectedMarker = (NTMarker*)overlay;
    } else {
...
-(void)refreshAnnotation:(AkylasCartoAnnotationProxy*)proxy readd:(BOOL)yn
{
    ...
    if (!IS_OF_CLASS(newSelected, NTMarker)) return;
//    NTMarker *selected = map.selectedMarker;
...
//        if (wasSelected)
//        {
//            map.selectedMarker = (NTMarker*)newSelected;
//        }
//    }
    [((NTMarker*)newSelected).layer setNeedsLayout];
...
-(void)internalRemoveAnnotations:(id)annotations
{
    TiThreadPerformBlockOnMainThread(^{
        ...
        if ([annotations isKindOfClass:[NSArray class]]) {
            for (AkylasCartoAnnotationProxy* annotProxy in annotations) {
...
//            return;
        ...
        if (IS_OF_CLASS(overlay, NTMarker)) {
            size = ((NTMarker*)overlay).layer.frame.size;
        }
...
//                CGFloat bearing  = location.course;
        ...
        {
            [self.proxy fireEvent:@"location" withObject:[AkylasCartoModule dictFromLocation:location] propagate:NO checkForListener:NO];
        }
        ...
        if (IS_OF_CLASS(newMarker, NTMarker)) {
...
- (BOOL)internalAddTileSources:(id)tileSource atIndex:(NSInteger)index
{
    TiThreadPerformBlockOnMainThread(^{
        ...
        if (IS_OF_CLASS(tileSource, NSArray)) {
            ...
            GMSMapView* mapView = [self map];
            GMSTileLayer* layer = [(AkylasCartoTileSourceProxy*)tileSource getGTileLayerForMapView:mapView];
...
- (BOOL)internalRemoveTileSources:(id)tileSource
{
    TiThreadPerformBlockOnMainThread(^{
        if (IS_OF_CLASS(tileSource, NSArray)) {
            ...
            [[(AkylasCartoTileSourceProxy*)tileSource gTileLayer] setMap:nil];
        }
    }, NO);
}
...
- (BOOL)internalAddClusters:(id)cluster atIndex:(NSInteger)index
{
    TiThreadPerformBlockOnMainThread(^{
        ...
        if (IS_OF_CLASS(cluster, NSArray)) {
            ...
            AkylasCartoClusterProxy* clusterProxy = (AkylasCartoClusterProxy*)cluster;
            [[[self map] getLayers] add:[clusterProxy getLayer]];
...
- (BOOL)internalRemoveClusters:(id)cluster
{
    TiThreadPerformBlockOnMainThread(^{
       if (IS_OF_CLASS(cluster, NSArray)) {
            ...
            AkylasCartoClusterProxy* clusterProxy = (AkylasCartoClusterProxy*)cluster;
            [((AkylasCartoClusterRenderer*)[_clusterManager clusterRenderer]) clearCacheForId:[clusterProxy uniqueId]];
...
-(BOOL)fireSelectedEventFromOverlay:(GMSOverlay*)oldOverlay toOverlay:(GMSOverlay*)newOverlay
{
    ...
    if ([self.viewProxy _hasListeners:@"selected" checkParent:NO])
    {
        ...
        if (IS_OF_CLASS(newOverlay, NTMarker)) {
            coords = ((NTMarker*)newOverlay).position;
        } else if (IS_OF_CLASS(oldOverlay, NTMarker)) {
            coords = ((NTMarker*)oldOverlay).position;
        }
        ...
        [event addEntriesFromDictionary:[AkylasCartoModule dictFromLocation2D:coords]];
        if (IS_OF_CLASS(newOverlay, AkylasClusterMarker)) {
            ...
            [event setObject:[AkylasCartoModule dictFromRegion:((AkRegion){.northEast = bounds.northEast, .southWest = bounds.southWest})] forKey:@"region"];
        } else if (annotProxy){
...
- (BOOL)fireEvent:(NSString*)type onOverlay:(GMSOverlay *) overlay source:(NSString *)source
{
    ...
    if (IS_OF_CLASS(annotProxy, AkylasMapBaseRouteProxy)) {
        ...
        [event setObject:annotProxy forKey:@"route"];
        CGFloat tolerance = [((AkylasCartoRouteProxy*)annotProxy) lineWidth]*_mpp/2;
        NSDictionary* result = [((AkylasCartoRouteProxy*)annotProxy) pointOnPathFrom:_touchCoords tolerance:tolerance];
        if (result) {
            [event setObject:result forKey:@"onpath"];
        }
    } else {
        CGPoint point = [[map projection] pointForCoordinate:((NTMarker*)overlay).position];
        event = [TiUtils dictionaryFromPoint:point inView:map];
        [event addEntriesFromDictionary:[AkylasCartoModule dictFromLocation2D:[[map projection] coordinateForPoint:point]]];
    ...
    [event addEntriesFromDictionary:[AkylasCartoModule dictFromLocation2D:_touchCoords]];
...
- (void)fireEventOnMap:(NSString*)type atCoordinate:(CLLocationCoordinate2D)coordinate
{
    ...
    if ([self.viewProxy _hasListeners:type checkParent:propagate]) {
        ...
        [event addEntriesFromDictionary:[AkylasCartoModule dictFromLocation2D:coordinate]];
        [event setObject:@(_mpp) forKey:@"mpp"];
        [event setObject:@(_internalZoom) forKey:@"zoom"];
...
//    }
    NTMapPos* pos = [mapClickInfo getClickPos];
    NTClickType type = [mapClickInfo getClickType];
    [self fireEventOnMap:@"click" atCoordinate:pos];
    [self fireEventOnMap:@"singletap" atCoordinate:pos];
    if (self.selectOnTap) {
        if (_selectedOverlay) {
            [self onDeselected:_selectedOverlay];
            [self fireSelectedEventFromOverlay:_selectedOverlay toOverlay:nil];
 ...
 */
- (BOOL)mapView:(GMSMapView *)mapView didTapMarker:(NTMarker *)marker {
 ...
 */
- (void)mapView:(GMSMapView *)mapView didTapInfoWindowOfMarker:(NTMarker *)marker {
...
- (void) mapView:(GMSMapView *) mapView didTapPOIWithPlaceID:(NSString *) placeID
    ...
    if ([self.viewProxy _hasListeners:@"poi" checkParent:NO])
    {
        ...
        [event addEntriesFromDictionary:[AkylasCartoModule dictFromLocation2D:location]];
 ...
 */
- (UIView *)mapView:(GMSMapView *)mapView markerInfoWindow:(NTMarker *)marker {
...
- (UIView *)mapView:(GMSMapView *)mapView markerInfoContents:(NTMarker *)marker {
    return nil;
}

-(void)didDragMarker:(NTMarker *)marker withState:(AkMapDragState)newState {
 ...
 */
- (void)mapView:(GMSMapView *)mapView didBeginDraggingMarker:(NTMarker *)marker {
    _dragging = YES;
 ...
 */
- (void)mapView:(GMSMapView *)mapView didEndDraggingMarker:(NTMarker *)marker {
    _dragging = NO;
 ...
 */
- (void)mapView:(GMSMapView *)mapView didDragMarker:(NTMarker *)marker {

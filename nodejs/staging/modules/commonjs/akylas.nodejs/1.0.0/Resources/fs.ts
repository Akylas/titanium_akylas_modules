namespace fs {
    ...
    var MODE_MAP = {};
    MODE_MAP['r'] = MODE_MAP['rs'] = $F.MODE_READ;
    MODE_MAP['w'] = MODE_MAP['wx'] = $F.MODE_WRITE;
    MODE_MAP['a'] = MODE_MAP['ax'] = $F.MODE_APPEND;
    MODE_MAP['r+'] = MODE_MAP['rs+'] = $F.MODE_READ | $F.MODE_WRITE;
    MODE_MAP['w+'] = MODE_MAP['wx+'] = $F.MODE_WRITE | $F.MODE_READ;
    MODE_MAP['a+'] = MODE_MAP['ax+'] = $F.MODE_APPEND | $F.MODE_READ;
    ...
    fs.readFile = function readFile(path, options, callback_) {
        ...
        // console.log('fs read file', path, options);
        setTimeout(function () {
            var err = null,
                res;
            try {
    ...
    fs.readFileSync = function readFileSync(path, options) {
        ...
        return readBlob(file.read(), encoding);
    };
    ...
    fs.readSync = function readSync(fd, buffer, offset, length, position) {
        ...
        if (Array.isArray(buffer)) {
            tiBuf = Ti.createBuffer({ value: buffer });
        }
        let result;
        result = fd.read(tiBuf, offset, length, position);
        if (tiBuf !== buffer) {
            let toWrite = tiBuf.byteArray;
    ...
    fs.write = function write(fd, buffer, offset, length, position, callback) {
        ...
        //     buffer = Ti.createBuffer({ value: buffer });
        // }
    ...
    // if (IS_ANDROID) {
        fs.writeSync = function writeSync(fd, buffer, offset, length, position) {
            // console.log('writeSync', fd, buffer, offset, length, position);
    ...
    fs.rename = function rename(oldPath, newPath, callback) {
        setTimeout(function () {
            var err = null,
                good = false;
            try {
                good = fs.renameSync(oldPath, newPath);
                if (!good) {
                    err = new Error('could not move file from ' + oldPath + ' to ' + newPath);
                }
            } catch (e) {
                err = e;
            }
    ...
    fs.renameSync = function renameSync(oldPath, newPath) {
        ...
        return $F.getFile(oldPath).move(newPath);
    };
    ...
    fs.writeFileSync = function writeFileSync(path, data, options) {
        ...
        $F.getFile(path).write(data);
            // fd = fs.openSync(path, 'w');
        // console.log('writeFileSync', path, data, options, fd);
    ...
    fs.appendFileSync = function appendFileSync(path, data, options) {
        // console.log('appendFileSync', path, data);
        options = options || {};
        var encoding = options.encoding || 'utf8',
    ...
    function readBlob(blob, encoding) {
        if (!encoding) {
            return blob;
        }
        switch (encoding.toLowerCase()) {
            case 'ascii':
            case 'binary':
                var byteArray = blob.byteArray;
                var ret = '';
                for (var i = 0; i < byteArray.length; i++) {
                    ret += String.fromCharCode(byteArray);
                }
                return ret;
            case 'utf8':
            case 'utf16':
            ...
            case 'utf-16':
                return blob.text;
            case 'base64':
                return Ti.Utils.base64encode(blob.text).toString();
            default:
                throw new Error('Unknown encoding: ' + encoding);
        }
    }

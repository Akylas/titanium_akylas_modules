...
export class AKApp extends TiEventEmitter {
    ...
    constructor(context, _args?) {
        ...
        if (_args.iconicfonts) {
            var IconicFont = akRequire('IconicFont').IconicFont;
            for (var name in _args.iconicfonts) {
                this.iconicfonts[name] = new IconicFont({
                    font: _args.iconicfonts[name]
                });
            }
        ...
        //we add _ functions
        if (context['_']) {
            var oldRemove = context._.remove;
            context._.mixin({
                remove: function (array, predicate, thisArg) {
                    if (_.isPlainObject(array)) {
                        ...
                        oldRemove(array, predicate, thisArg);
                    }
                },
        ...
        if (_args.mappings) {
            ...
            for (var key in _args.mappings) {
                ...
                ak.ti.redux.fn.addNaturalConstructor(context, source, value[1], key);
            }
    ...
    share(_args, _externalImage) {
        ...
        if (__APPLE__) {
            ...
            Ti.UI.iOS.createActivityView({
                ...
                items: items,
                subjectForActivityType: hasSubjectForActivity ? (function (_type, _items) {

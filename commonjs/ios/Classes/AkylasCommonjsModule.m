...
-(void)startup
{
    ...
    if (!commonjsKey) {
        [self throwException:@"You need to set the \"akylas.commonjs.key\"" subreason:nil location:CODELOCATION];
   }
    ...
    if (![[commonjsKey lowercaseString] isEqualToString:result]) {
        [self throwException:@"wrong \"akylas.commonjs.key\" key!" subreason:nil location:CODELOCATION];
    }

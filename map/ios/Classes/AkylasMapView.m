...
MKCoordinateRegion mkregionFromRegion(AkRegion trapez)
{
    return (MKCoordinateRegion){
        .center = AkRegionCenter(trapez),
        .span = {
...
AkRegion regionFromMKRegion(MKCoordinateRegion mkregion)
{
...
{
    return [AkylasMapModule dictFromLocation2D:AkRegionCenter(region)];
}
...
-(void)setCamera_:(AkylasMapCameraProxy*)value
{
    [self map].camera = [value camera];
}
-(AkylasMapCameraProxy*)camera
{
    return [[[AkylasMapCameraProxy alloc] initWithCamera:[self map].camera] autorelease];
}
...
-(void)animateCamera:(id)args
{
    ...
    ENSURE_TYPE(cameraProxy, AkylasMapCameraProxy);
...
- (void)mapView:(MKMapView *)mapView didSelectAnnotationView:(MKAnnotationView *)annotationView {
    ...
    if (canShowCallout) {
        ...
        if ([annotationView respondsToSelector:@selector(setCalloutView:)]) {
            [(id)annotationView setCalloutView:_calloutView];
        }

...
@implementation AkylasCartoRouteProxy
{
...
-(void)showInfo:(id)args
{
    if (self.showInfoWindow) {
        if (_gPoly.map && IS_OF_CLASS(_gPoly.map.delegate, AkylasCartoView)) {
            [ (AkylasCartoView*)(_gPoly.map.delegate) showCalloutForOverlay:_gPoly];
        }
    }
...
{
    if (_gPoly.map && IS_OF_CLASS(_gPoly.map.delegate, AkylasCartoView)) {
        [ (AkylasCartoView*)(_gPoly.map.delegate) hideCalloutForOverlay:_gPoly];
    }
}

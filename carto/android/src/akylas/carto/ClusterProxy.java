...
//                public void execute() {
//                    ((CartoView) mapView).getClusterManager().clusterAlgo(_algorithm);
//                }
//            }, false);
        }
    }
...
//            getOrCreateAlgorithm().addItems(toAdd);
            ...
            for (Object item : toAdd) {
                if (item instanceof AnnotationProxy) {
                    ...
                    getSource().add(((CartoView) this.mapView).addAnnotationToLayer((AnnotationProxy) item, source));
                }
            }
            cluster();
        }
    ...
    protected void onAnnotationsRemoved(List removed) {
        for (Object item : removed) {
            if (item instanceof AnnotationProxy) {
                ((CartoView) this.mapView).handleRemoveSingleAnnotation((AnnotationProxy) item);
            }
        }
...
//            markerBitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.marker_black);
    ...
    public void removeFromMap() {
        if (layer != null && mapView != null) {
            ...
            source = null;
            layer = null;
//            ((CartoView) mapView).getClusterManager().removeClusterAlgorithm(_algorithm);
        }
    }
    private LocalVectorDataSource getSource() {
        if (source == null) {
            source = new LocalVectorDataSource(((CartoView) this.mapView).getProjection());   
        }
        return source;
    }
    ...
    public Layer getLayer() {
        if (layer == null) {
            layer = new ClusteredVectorLayer(getSource(), new MyClusterElementBuilder());
            layer.setVisible(visible);
        }
        return layer;
    }

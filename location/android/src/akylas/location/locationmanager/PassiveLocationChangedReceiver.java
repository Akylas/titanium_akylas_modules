...
import akylas.location.locationmanager.LegacyLastLocationFinder;
...
public class PassiveLocationChangedReceiver extends BroadcastReceiver {
	...
	protected static String TAG = "AkylasPassiveLocationChangedReceiver";
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.hasExtra(key)) {
			...
			location = lastLocationFinder.getLastBestLocation(Manager.desiredAccuracy, System.currentTimeMillis()
					- PlacesConstants.MAX_TIME);
			SharedPreferences prefs = context.getSharedPreferences(PlacesConstants.SHARED_PREFERENCE_FILE,
		...
		if (location != null) {
			Log.d(TAG, "Passivly updating:" + location.toString(), Log.DEBUG_MODE);
			Manager.handleNewLocation(location);
		}
	}
}
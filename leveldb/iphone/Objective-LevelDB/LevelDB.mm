...
- (NSError*) setObject:(id)value forKey:(id)key {
    AssertDBExists(db);
    AssertKeyType(key);
    ...
    NSError* error;
    if (!status.ok()) {
        error = [NSError errorWithDomain:[NSString stringWithUTF8String:status.ToString().c_str()] code:(NSInteger)status.getCode() userInfo:nil];
    }
    
    return [error autorelease];
}
- (NSError*) setValue:(id)value forKey:(NSString *)key {
    return [self setObject:value forKey:key];
}
- (NSError*) setObject:(id)value forKeyedSubscript:(id)key {
    return [self setObject:value forKey:key];
}
- (NSError*) addEntriesFromDictionary:(NSDictionary *)dictionary {
    __block NSError* error;
    [dictionary enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        error = [self setObject:obj forKey:key];
        if (error) {
            *stop = YES;
        }
    }];

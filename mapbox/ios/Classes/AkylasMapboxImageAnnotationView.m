...
@implementation AkylasMapboxImageAnnotationView


- (id)initWithAnnotation:(id <MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier map:(AkylasMapboxView*)map_  image:(UIImage*)image
{
		...
		for (UIView * ourSubView in [self subviews])
		{
			...
			for (UIView * ourSubSubView in [ourSubView subviews])
			{
					...
					[ourSubSubView isKindOfClass:[UILabel class]])
				{
					...
					AkylasMapboxAnnotationProxy * ourProxy = (AkylasMapboxAnnotationProxy *)[self annotation];
					RELEASE_TO_NIL(lastHitName);

...
@implementation AkylasMapGoogleMapView
{
    ...
    RMMapLayerDragState _dragState;
}
...
-(void)dealloc
{
	if (map!=nil)
	{
        ...
        if (_userTrackingMode != RMUserTrackingModeNone) {
...
-(void)setRegion_:(id)value
{
    ...
    GMSCameraPosition* position = [[self map] cameraForBounds:boundsFromRegion([AkylasMapModule regionFromObject:value]) insets:UIEdgeInsetsZero];
//    if (!configurationSet) {
...
-(id)userLocation_
{
    return [AkylasMapModule dictFromLocation:[self map].myLocation];
}
...
-(void)setUserTrackingMode_:(id)value
{
    ...
    _userTrackingMode = [TiUtils intValue:value def:RMUserTrackingModeNone];
...
{
    NSUInteger type = [TiUtils intValue:value def:MKMapTypeStandard];
    switch (type) {
...
-(void)setCenterCoordinate_:(id)value
{
    ENSURE_UI_THREAD_1_ARG(value)
    [self setShouldFollowUserLocation:NO];
    CLLocationCoordinate2D coord = [AkylasMapModule locationFromObject:value];
    if (animate || !configurationSet) {
...
{
    return [AkylasMapModule dictFromLocation2D:[self centerCoordinate]];
}
...
-(void)setShouldFollowUserLocation:(BOOL)value
{
    if (_shouldFollowUserLocation != value) {
        ...
        {
            [self.proxy fireEvent:@"usertracking" withObject:@{@"mode":NUMINTEGER(_shouldFollowUserLocation?_userTrackingMode : RMUserTrackingModeNone), @"animated":NUMBOOL(animate)} propagate:NO checkForListener:NO];
        }
    }
}
...
-(RMSphericalTrapezium) getCurrentRegion
{
    ...
    return ((RMSphericalTrapezium){.northEast = {.latitude = visible.farRight.latitude, .longitude = visible.farRight.longitude}, .southWest = {.latitude = visible.nearLeft.latitude, .longitude = visible.nearLeft.longitude}});
}
...
-(void)internalAddAnnotations:(id)annotations atIndex:(NSInteger)index
{
    ...
    if (IS_OF_CLASS(annotations, NSArray)) {
        ...
        GMSMapView* mapView = [self map];
        GMSOverlay* overlay = [(AkylasMapAnnotationProxy*)annotations getGOverlayForMapView:mapView];
        if (((AkylasMapAnnotationProxy*)annotations).zIndex == -1) {
            overlay.zIndex = (int)(realIndex + [[annotations class] gZIndexDelta]);
        } else {
            overlay.zIndex = (int)((AkylasMapAnnotationProxy*)annotations).zIndex;
        }
    ...
    if ([annotations isKindOfClass:[NSArray class]]) {
        for (AkylasMapAnnotationProxy* annotProxy in annotations) {
            if ([annotProxy isKindOfClass:[AkylasMapAnnotationProxy class]]) {
                [[annotProxy gOverlay] setMap:nil];
            }
        }
    }
    else if ([annotations isKindOfClass:[AkylasMapAnnotationProxy class]]) {
        [[(AkylasMapAnnotationProxy*)annotations gOverlay] setMap:nil];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"myLocation"]) {
        ...
        if (_shouldFollowUserLocation && _userTrackingMode != RMUserTrackingModeNone) {
...
//            GMSMutablePosition* currentposition = [self map].camera;
        ...
        {
            [self.proxy fireEvent:@"location" withObject:[AkylasMapModule dictFromLocation:location] propagate:NO checkForListener:NO];
        }
...
- (BOOL)internalAddTileSources:(id)tileSource atIndex:(NSInteger)index
{
    ...
    if (IS_OF_CLASS(tileSource, NSArray)) {
        ...
        GMSMapView* mapView = [self map];
        GMSTileLayer* layer = [(AkylasMapTileSourceProxy*)tileSource getGTileLayerForMapView:mapView];
        [layer setMap:mapView];
        if (((AkylasMapTileSourceProxy*)tileSource).zIndex == -1) {
            layer.zIndex = (int)realIndex;
        } else {
            layer.zIndex = (int)((AkylasMapTileSourceProxy*)tileSource).zIndex;
        }
...
- (BOOL)internalRemoveTileSources:(id)tileSource
{
    if (IS_OF_CLASS(tileSource, NSArray)) {
        ...
        [[(AkylasMapTileSourceProxy*)tileSource gTileLayer] setMap:nil];
    }
}
...
- (BOOL)fireEvent:(NSString*)type onOverlay:(GMSOverlay *) overlay source:(NSString *)source
{
    ...
    AkylasMapAnnotationProxy *annotProxy = IS_OF_CLASS(overlay, GMSMarker)? ((GMSMarker*)overlay).userData: nil;
    if (annotProxy == nil)
    {
        return NO;
    }
    ...
    [event addEntriesFromDictionary:[AkylasMapModule dictFromLocation2D:[[map projection] coordinateForPoint:point]]];
    if (IS_OF_CLASS(annotProxy, AkylasMapRouteProxy)) {
...
- (void)fireEventOnMap:(NSString*)type atCoordinate:(CLLocationCoordinate2D)coordinate
{
    ...
    if ([self.viewProxy _hasListeners:type checkParent:NO]) {
        ...
        [event addEntriesFromDictionary:[AkylasMapModule dictFromLocation2D:coordinate]];
...
//    region = regionFromMKRegion([mapView region]);
    if (mapView.selectedMarker != nil && _calloutView && !_calloutView.hidden) {
        ...
        AkylasMapAnnotationProxy *annProxy = marker.userData;
...
//        [_calloutView presentCalloutFromRect:calloutRect inLayer:marker.layer constrainedToLayer:map.layer animated:YES];
        [_calloutView presentCalloutFromRect:calloutRect inView:vectorMap constrainedToView:map animated:YES];
...
- (void)mapView:(GMSMapView *)mapView didTapInfoWindowOfMarker:(GMSMarker *)marker {
    if ([marker.userData isKindOfClass:[AkylasMapAnnotationProxy class]])
    {
        AkylasMapAnnotationProxy* proxy = marker.userData;
//        NSString * clickSource = @"unknown";
//        if (marker.leftCalloutAccessoryView == control)
//        {
//            clickSource = @"leftButton";
...
-(void)didDragMarker:(GMSMarker *)marker withState:(RMMapLayerDragState)newState {
    AkylasMapAnnotationProxy* proxy = marker.userData;
...
- (void)mapView:(GMSMapView *)mapView didBeginDraggingMarker:(GMSMarker *)marker {
    [self didDragMarker:marker withState:RMMapLayerDragStateStarting];
}
...
- (void)mapView:(GMSMapView *)mapView didEndDraggingMarker:(GMSMarker *)marker {
    [self didDragMarker:marker withState:RMMapLayerDragStateEnding];
}
...
- (void)mapView:(GMSMapView *)mapView didDragMarker:(GMSMarker *)marker {
    [self didDragMarker:marker withState:RMMapLayerDragStateDragging];
}

...
- (BOOL)internalAddTileSources:(id)tileSource atIndex:(NSInteger)index
{
    ...
    if (IS_OF_CLASS(tileSource, NSArray)) {
        ...
        RMMapView* mapView = [self map];
        if (_userStackTileSource) {
            [[self getTileSourceContainer] addTileSource:[(AkylasMapTileSourceProxy*)tileSource getMPTileSourceForMapView:mapView] atIndex:realIndex];
            if ([[mapView tileSources] count] == 0) {
                [mapView setTileSource:_tileSourceContainer];
            }
        } else {
            [mapView addTileSource:[(AkylasMapTileSourceProxy*)tileSource getMPTileSourceForMapView:mapView] atIndex:realIndex];
        }
    }
}
...
- (BOOL)internalRemoveTileSources:(id)tileSource
{
    if (IS_OF_CLASS(tileSource, NSArray)) {
        ...
        if (_userStackTileSource) {
            [_tileSourceContainer removeTileSource:[(AkylasMapTileSourceProxy*)tileSource mpTileSource]];
        } else {
            [[self map] removeTileSource:[(AkylasMapTileSourceProxy*)tileSource mpTileSource]];
        }
    }
}

...
@Kroll.proxy(creatableInModule=AkylasMapboxModule.class, propertyAccessors = {
	...
	AkylasMapboxModule.PROPERTY_DRAGGABLE,
	TiC.PROPERTY_IMAGE,
	TiC.PROPERTY_PINCOLOR,
	AkylasMapboxModule.PROPERTY_CUSTOM_VIEW,
...
public class AnnotationProxy extends KrollProxy
{
	...
	public AnnotationProxy()
	{
		super();
		annoTitle = "";
//		defaultValues.put(AkylasMapboxModule.PROPERTY_SHOW_INFO_WINDOW, true);
	}
...
//		marker.getMarker().setPosition(position);
	...
	public void handleCreationDict(KrollDict dict) {
		...
		if (dict.containsKey(TiC.PROPERTY_LEFT_BUTTON) || dict.containsKey(TiC.PROPERTY_LEFT_VIEW)
			|| dict.containsKey(TiC.PROPERTY_RIGHT_BUTTON) || dict.containsKey(TiC.PROPERTY_RIGHT_VIEW)
			|| dict.containsKey(TiC.PROPERTY_TITLE) || dict.containsKey(TiC.PROPERTY_SUBTITLE)) {
...
//				infoWindow.setSubtitle(null);
		...
		if (dict.containsKey(AkylasMapboxModule.PROPERTY_DRAGGABLE)) {
//			markerOptions.draggable(TiConvert.toBoolean(getProperty(AkylasMapboxModule.PROPERTY_DRAGGABLE)));
		}

		// customView, image and pincolor must be defined before adding to mapview. Once added, their values are final.
		if (dict.containsKey(AkylasMapboxModule.PROPERTY_CUSTOM_VIEW)) {
			handleCustomView(dict.get(AkylasMapboxModule.PROPERTY_CUSTOM_VIEW));
...
//	}
	...
	{
		if (marker == null) {
			marker = new TiMarker(TiConvert.toString(getProperty(TiC.PROPERTY_TITLE)), 
					TiConvert.toString(getProperty(TiC.PROPERTY_SUBTITLE)), AkylasMapboxModule.latlongFromDict(getProperties()));
		}
		return marker;
	}
...
//			getOrCreateMapInfoWindow().setLeftOrRightPane(value, TiMapInfoWindow.RIGHT_PANE);
			updateInfoWindow();
		} else if (name.equals(AkylasMapboxModule.PROPERTY_DRAGGABLE)) {
